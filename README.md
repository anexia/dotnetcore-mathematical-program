...
- Install latest package of `Anexia.MathematicalProgram` via nuget
...
- To build the objective function of your LP/ILP you can use the class `Terms : IEnumerable<Term>`.
  Each `Term` is defined by a `Coefficient` and a variable of type `Google.OrTools.LinearSolver.Variable`.
  Moreover, there is the possibility to have an additional `Constant` (not linked to a variable).

- To build your constraints you can use the class `Constraints : IEnumerable<Constraint>`.
  Each `Constraint` is defined by `Terms` and an `Interval : IInterval` that has a lower and an upper bound of
  type `double`.
  For binary intervals simply use `Interval.BinaryInterval`.
  Another implementation is the class `Point : IInterval` for the case of lower bound equals upper bound.
